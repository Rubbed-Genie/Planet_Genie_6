...
namespace ntw.CurvedTextMeshPro
{
    ...
    public class TextProOnACylinder : TextProOnACurve
    {
        ...
        [SerializeField]
        [Tooltip("Toggle to enable or disable gradual decrement of angular offset")]
        private bool enableDecrement = false;

        [SerializeField]
        [Tooltip("Amount to decrement angular offset by")]
        private float decrementAmount = 1.0f;

        [SerializeField]
        [Tooltip("Time interval between each decrement")]
        private float decrementInterval = 1.0f;
        ...
        protected override bool ParametersHaveChanged()
        {
            ...
            if (enableDecrement && Application.isPlaying)
            {
                // Gradually decrement angular offset
                m_angularOffset -= decrementAmount * Time.deltaTime / decrementInterval;
            }
